...
(function() {
  ...
  let tagFilter = null;
  let registerElementCalled = false;
  ...
  document.registerElement = function(tagName, options) {
    registerElementCalled = true;

    if (tagFilter && !tagFilter.test(tagName)) {
      return originalRegisterElement.apply(this, arguments);
    }
    ...
    return forkStatsZone(tagZone, callbackZoneStack, 'register').run(function() {
  ...
  function setFilterString(filterString) {
    window.localStorage.setItem('filterString', JSON.stringify({
      filterString: filterString
    }));

    if (registerElementCalled) {
      window.location.reload();
      return;
    }

    if (filterString != null) {
      tagFilter = new RegExp(filterString);
    } else {
      tagFilter = null;
    }
  }
  ...
  window.addEventListener('message', function(event) {
    switch(event.data.messageType) {
      case 'filter-tags':
        setFilterString(event.data.detail);
        break;
      case 'get-element-stats':
        event.source.postMessage({
          messageType: 'element-stats',
          data: gatherStats(),
        }, '*');
        break;
      case 'clear-element-stats':
        clearStats();
        event.source.postMessage({
          messageType: 'element-stats-clear',
        }, '*');
        break;
    }
  });
  ...
  try {
    setFilterString(JSON.parse(window.localStorage.getItem('filterString')).filterString);
  } catch (e) {
